...
export function matchModuleToTerm (module, value) {
  return (
    module.name.toLowerCase().indexOf(value.toLowerCase()) !== -1 ||
    module.abbr.toLowerCase().indexOf(value.toLowerCase()) !== -1
  );
}

export function request (value, cb) {
  if (value === '')
    return getModules();
  let items = getModules().filter((module) => {
    return matchModuleToTerm(module, value);
  });
  setTimeout(() => {
    cb(items);
  }, 500);
...
export function getModules() {
  return [
    { id: 1, abbr: "WE", name: "Weather"},
    { id: 2, abbr: "CL", name: "Clock"},
    { id: 3, abbr: "SL", name: "Commute"}
  ];
}
...
export function fakeRequest (value, cb) {
  if (value === '')
    return getStates();
  let items = getStates().filter((state) => {
